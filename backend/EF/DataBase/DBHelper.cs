...
namespace WhiteBoard.EF.DataBase
{
    ...
    internal sealed class DBHelper
    {
        ...
        private DbParameterBuilder _dbParameterBuilder = null;
        ...
        internal async Task<IDataReader> GetDataReader(string sql) => await GetDataReader(sql, null);
        /// <summary>
        /// 执行查询语句,单行
        /// </summary>
        /// <param name="sql">执行语句</param>
        ...
        /// <returns>DbDataReader对象</returns>
        internal async Task<IDataReader> GetDataReader(string sql, IDictionary<string, object> dicParams)
        {
            using (DbCommand cmd = await CreateDbCommand(sql, dicParams))
            {
                try
